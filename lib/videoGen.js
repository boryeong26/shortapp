...
// Same as generateSceneClip, but retries on failure/timeout instead of
// giving up after a single attempt. Each retry uses a fresh
// external_task_id (Kling dedupes by that id, so re-using the same one
// after a timeout would just re-attach to the same slow/stuck task).
async function generateSceneClipWithRetry(prompt, baseExternalTaskId, options = {}) {
  const maxRetries = options.maxRetries || 2; // total up to 3 attempts
  let lastErr;

  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    const externalTaskId = attempt === 0 ? baseExternalTaskId : `${baseExternalTaskId}-retry${attempt}`;
    try {
      return await generateSceneClip(prompt, externalTaskId, options);
    } catch (err) {
      lastErr = err;
      if (attempt < maxRetries) {
        console.warn(`     클립 생성 실패 (시도 ${attempt + 1}/${maxRetries + 1}), 재시도합니다: ${err.message}`);
      }
    }
  }
