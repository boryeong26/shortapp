...
function parseScriptSegments(script) {
  ...
  const missing = SEGMENT_ORDER.filter((label) => !nonEmpty.some((seg) => seg.label === label));
  if (missing.length > 0) {
    throw new Error(
      `대본에 다음 섹션이 없습니다: ${missing.join(', ')}. 대본이 중간에 잘렸을 수 있습니다. ` +
      '"1. 새 대본 생성하기"로 다시 생성한 뒤 영상 제작을 시도해주세요.'
    );
  }
